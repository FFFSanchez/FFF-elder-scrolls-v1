				...
				try:
					...
					with open(sg.download_image(nfo['image']), 'rb') as photo:
						for s in subscriptions:
							print(s)
							await bot.send_photo(s[1], photo, caption = nfo['title'] + '\n' + 'Score: ' + nfo['score'] + '\n' + nfo['except'] + '\n\n' + nfo['link'], disable_notification = False)'''
					
					for s in subscriptions: # работает только если открывать для каждого подписчика отдельно
						print(s)
						with open(sg.download_image(nfo['image']), 'rb') as photo:
							await bot.send_photo(s[1], photo, caption = nfo['title'] + '\n' + 'Score: ' + nfo['score'] + '\n' + nfo['except'] + '\n\n' + nfo['link'], disable_notification = False)
					
				except:
					print('Error code zalupa')
